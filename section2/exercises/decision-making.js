...
If the condition was evaluated and found to not be strictly equal or false, the script will continue with a different statement
that assigns the bearClothing variable a string value of scarf.

2. What variable has a new value assigned to it after the first if statement executes?

Once the statement executes, the bearClothing variable will have it's empty string value reassigned
to the string value hat since it confirms that the value of doorChoice is stricly equal to the the number
...
essentially meaning that if the first statement is evaluated to be not true, the second script
will be true by default and will run. The value of the bearClothing variable can only be hat if
...
and will be evaluated against the condition. The statement will be ignored if found to be
false until a line is found to be true, then it will execute the script for that code. It
first asks us to compare the value of the bearChoice variable to the number 1 for strict
equality. We can see that the bearChoice variable was assigned the value 1, which is strictly equal
to the other operand 1. Since the first statement is evaluated as true it will initiate
the first script to run. If it were false, the subsequent statements would then be
evaluated in order and executed if they were found to be true. Only one if/else if/else statement
needs to be true and Javascript will ignore all other instructions once
...
assigned a string value of hat, as defined in the condition of the first if/else statement on lines 14 through 18.
With a bearChoice variable equal to 2, the if statement would be evaluted as false, skipping it
and moving on tothe next condition to be evaluated. The first else/if statement would be
evaluated as true since 2 is strictly equal to 2 and it would print the script that follows.
Through concatenation, the variable bearClothing's string value of hat is added into the string and
